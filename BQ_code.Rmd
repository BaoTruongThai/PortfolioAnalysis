...
Needed libraries for this assignment
...
```
- Ten stocks would be used for this assignment satisfying some categories:
  * Diversifying the portfolio with 10 different industries
  * The large capital and liquidity of each stock
  
- The period of time is 6 years
```{r pressure}
...
Getting datasets for 10 stocks, All Ordinaries, 3-months BABs/NCDs:
  
  The data retrieved (including 10 stocks and All Ordinaries) would be converted from daily price into monthly returns
```{r}
...
```
  ...
  The data retrieved for 3-months BABs/NCDs as usually measured as annual interest rate (12 months) metric so we need to transform into interest rate of 3-months by divided (1200 = 12 months * 100 rate converted from percent)
```{r}
...
```
  ...
  Data of three elements is merged into 1 dataframe
```{r}
# Step 4: Combine data
all_returns <- do.call(merge, c(stock_return_list, list(Market = aord_monthly_returns), list(Rf = Rf$value)))
...
regression_table <- regression_results %>%
  mutate(
    ...
    H0_Beta = ifelse(Beta_p_value < 0.05, "Reject", "Do not reject") #C Nowers
  )
...
```{r}
# 5.7 Diagnostic plots for one example stock (e.g., CBA.AX)
...
# Đảm bảo dữ liệu đã sắp xếp theo thứ tự beta ước tính để biểu đồ dễ đọc hơn
beta_comparison <- beta_comparison %>%
  arrange(estimated_beta)

# Tạo biểu đồ dumbbell

#ggplot(beta_comparison, aes(y = reorder(symbol, estimated_beta))) +
#  geom_segment(aes(x = published_beta, xend = estimated_beta, 
#                   yend = symbol), 
#               color = "grey", size = 1) +
#  geom_point(aes(x = published_beta, color = "Published Beta"), size = 3) +
#  geom_point(aes(x = estimated_beta, color = "Estimated Beta"), size = 3) +
#  geom_text(aes(x = published_beta, label = round(published_beta, 2)), 
#            hjust = -0.5, size = 3) + 
#  geom_text(aes(x = estimated_beta, label = round(estimated_beta, 2)), 
#            hjust = 1.5, size = 3) +
#  scale_color_manual(values = c("Published Beta" = "blue", 
#                                "Estimated Beta" = "red")) +
#  labs(title = "Comparison of Published vs. Estimated Beta Values",
#       subtitle = "Horizontal segments show the difference between the two measures",
#       x = "Beta Value",
#       y = "",
#       color = "Beta Type") +
#  theme_minimal() +
...
ggplot(beta_comparison, aes(y = symbol)) +
  ...
  labs(
    ...
    x     = "Beta",
    y     = "Stock",
    caption = "Blue: Published Beta • Red: Estimated Beta "
  ) +
  theme_minimal() +
  theme(
...
The Min_Variance Optimized function and The portfolio with Max-Sharp optimized function
```{r}
...
The scatter compare portfolios with stock
```{r}
